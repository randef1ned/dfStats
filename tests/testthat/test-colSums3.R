for (i in seq_len(50)) {
  ...
  for (method in 1:3) {
    ...
    test_that("Test sum rows using apply", {
      expect_equal(unname(apply(data2, 2, sum)),
                   colSums3(data2, no_check = TRUE),
                   tolerance = 0.01)
    })
    test_that("Test sum rows using matrixStats", {
      expect_equal(matrixStats::colSums2(data1),
                   colSums3(data2, no_check = FALSE),
                   tolerance = 0.01)
    })
  }
}
...
test_that("Test NA", {
  ...
  expect_equal(unname(apply(data3, 2, sum)),
               colSums3(data3, no_check = TRUE),
               tolerance = 0.01)
  expect_equal(unname(apply(drop_na(data3), 2, sum)),
               colSums3(data3, no_check = TRUE, na.rm = TRUE),
               tolerance = 0.01)
})

test_that("Test warning", {
  data3 <- data.frame(a = 1:10, b = LETTERS[1:10], c = c(1:8, NA, 10))
  expect_warning(colSums3(data3))
})
